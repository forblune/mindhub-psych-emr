...
- **E2E (Playwright/Chromium, mock 모드)**: `npm test` → **20/20 통과, 콘솔 에러 0**
  - 렌더 / KPI 집계 / 환자 전환 / 탭4 / 테마 / 검색 / 정렬 / 노트 CRUD / 처방 CRUD / 척도 CRUD / 검사 CRUD
...
- **KPI 집계 뷰** — `0009_kpis_view.sql` `dashboard_kpis` 뷰(`security_invoker=true`)로 6개 지표 라이브 집계. RLS 적용 → 의사는 본인 환자, 간호사/관리자는 전체 기준. `getKpis()`가 뷰 조회, mock은 동일 정의로 큐에서 계산(`computeKpiRaw`). 기존 `kpis` 테이블은 레거시(미사용).
...
1. Realtime 확장 — 노트/처방까지 구독(현재 대기열만).
2. 새로고침·"신규 진료 시작" 버튼 실동작.
3. 입원·병동 모듈(담당 입원 KPI 복원), 예약 상태 모델링.
4. 화면 추가: 통계 지표, 청구·수납.
