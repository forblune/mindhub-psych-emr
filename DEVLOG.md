...
- **E2E (Playwright/Chromium, mock 모드)**: `npm test` → **21/21 통과, 콘솔 에러 0**
  - 렌더 / KPI 집계(7) / 입원·병동 네비·보드·필터 / 환자 전환 / 탭4 / 테마 / 검색 / 정렬 / 노트 CRUD / 처방 CRUD / 척도 CRUD / 검사 CRUD
...
1. Realtime 확장 — 노트/처방/입원까지 구독(현재 대기열만).
2. 새로고침·"신규 진료 시작" 버튼 실동작.
3. 입원 CRUD(현재 조회만 — 입퇴원 등록/이동), 예약 상태 모델링.
4. 화면 추가: 통계 지표, 청구·수납.
