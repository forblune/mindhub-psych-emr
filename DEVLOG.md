...
KPI 집계뷰 → 예약·청구 → 약품·재고(+처방 자동 차감·입출고 이력) →
진단 DSM-5→ICD-10(KCD) 표준화 → 예약→진료→청구 워크플로 연동 →
심리평가/척도 MVP 데모 검사 화면까지 완료.
**화면 8종(대시보드·심리평가/척도·예약·병동·통계·검색·청구·약품)**. 데이터는 전부 가상.
라이브 데모: https://forblune.github.io/psych-emr/
...
| 19 | **약품·재고 모듈** (입고·불출·등록·삭제) + **처방 시 재고 자동 차감** | ✅ |
| 20 | **진단 DSM-5→ICD-10(KCD) 입력** + 대기열·패널·병동·청구 한글 진단명 | ✅ |
| 21 | **통계 진단 분포 한글명화 + 진단군(F-블록) 집계 카드** | ✅ |
| 22 | **예약→진료→청구 워크플로 연동** (진료 시작 시 청구 자동 생성) | ✅ |
| 23 | **약품 입·출고 이력**(감사 로그 — 입고/불출/조제, 누가·언제·왜) | ✅ |
| 24 | **진단 다중 코드 확장(48종)** + 진단 선택기 **즐겨찾기·최근 사용** | ✅ |
| 25 | **문서**: MindHub×Psych-EMR 교차 분석 + 제품 로드맵(`docs/`) | ✅ |
| 26 | **심리평가/척도 MVP 데모** — PHQ-9·GAD-7·수면·자살위험 체크, 프론트 상태 점수 계산 | ✅ |
...
| `bc65427` | 청구·수납 모듈 |
| `8c509fe` | 약품·재고 모듈 |
| `77c10c6` | 처방 시 재고 자동 차감 연동 |
| `c19fb35` | 진단 입력 — DSM-5 선택 → ICD-10(KCD) 저장 |
| `d5db077` | 환자 패널·대기열에 KCD 한글 진단명 표시 |
| `32ece73` | 병동·청구 화면에도 KCD 한글 진단명 적용 |
| `5824f0a` | 통계 진단 분포에 KCD 한글명 표시 |
| `47f1edd` | 통계에 진단군(F-블록)별 집계 카드 추가 |
| `c14371f` | 예약→진료→청구 워크플로 연동 |
| `046fea6` | 약품 입·출고 이력(감사 로그) |

> 마이그레이션 `0001~0020`, E2E **40/40**, **화면 8종**(대시보드·심리평가/척도·예약·병동·통계·검색·청구·약품), 임상 데이터 4종 + 입원 CRUD + 신규 접수 + 예약·청구 + 약품·재고(입고·불출·등록·삭제, 처방 시 재고 자동 차감) + 담당의별 RLS + Realtime(큐·노트·처방·입원).
...
- 사이드바 "심리평가 · 척도" → PHQ-9/GAD-7/수면/자살위험 샘플 검사 모달, 점수 계산 + 참고 구간 표시
...
- **E2E (Playwright/Chromium, mock 모드)**: `npm test` → **40/40 통과, 콘솔 에러 0**
...
## 구현 완료 (2026-06-24)
- **심리평가/척도 MVP 데모 화면** (`src/components/ScaleDemo.jsx`) — 기존 환자 차트의 저장형 `ScalesTab`과 분리해, 사이드바 "심리평가 · 척도"에서 여는 포트폴리오용 샘플 검사 화면을 추가. 백엔드/DB 저장 없이 React state만 사용.
- **샘플 검사 기능** — PHQ-9(9문항), GAD-7(7문항), 수면 상태(4문항), 자살위험 체크(4문항)를 카드로 제공. 카드 클릭 → 모달, 문항별 선택지 클릭 → 총점 자동 계산.
...
- **진단 특정자/중증도 다중 코드 확장** (`data/diagnoses.js`) — 진단 마스터 28→**48종**. 흔한 진단의 중증도·정신병적 동반·아형 특정자를 다중 코드로 수록: 단일/재발 우울 4단계(F32.0~.3·F33.0~.3), 양극성 우울 에피소드(F31.3~.5), 조현병 아형(F20.1 파과·.2 긴장·.3 미분화·.5 잔류), 강박 하위(F42.0 사고·.1 행위·.2 혼합), 알코올 유해사용(F10.1), 공포/스트레스(F40.0·.2·F41.2·F43.0), 인격(F60.0·.1·.5). 코드·한글명은 kcdcode.kr/koicd.kr로 확인. note에 중증도 밴드 명시. 스키마 변경 없음(diagnoses 테이블 그대로) — seed만 재생성.
- **진단 선택기 즐겨찾기·최근 사용** (`lib/dxPrefs.js`, `DiagnosisPicker`) — 48종으로 늘어난 진단을 빠르게 고르도록 ★ 즐겨찾기 토글 + 최근 선택 자동 기록(localStorage, 사용자별·스키마 변경 없음). 검색어가 비면 **즐겨찾기 · 최근 사용 · 전체** 섹션으로 표시, 선택 시 최근에 prepend(최대 8). ⚠ 구현 중 발견·수정: 같은 code 가 여러 섹션에 동시 렌더되며 React **key 충돌**로 필터 전환 시 유령 행이 남던 버그 → 섹션별 key 접두사(`fav`/`recent`/`all`/`q`)로 해결. onFocus 에서 blur 타이머 취소(드롭다운 닫힘 경합 제거).
- **포트폴리오 문서 2종 추가** (`docs/`, 코드 무수정) — 다중 에이전트 워크플로로 자매 프로젝트 `forblune/mindhub-mvp`(환자측 마음기록)와 본 EMR을 교차 분석.
  - `docs/project-analysis.md`: 두 축 역할 · 현재 구조도 · MindHub→EMR 데이터 흐름(care_links + consent RPC) · 향후 통합 아키텍처(공유 Supabase) · 강점 · 기술부채 · 다음 우선순위. **적대적 검증 에이전트**가 통합 적색선(consent 강제 RLS→RPC 격하, red_flag↔동의 충돌, 식별자 무결성, 권한상승) 적발 → 교정 반영.
  - `docs/product-roadmap.md`: 6개월/1년 로드맵 · MVP→Beta→Production · 정신과 실도입 기술요소([보유·부분·미구현]) · 수익 모델. 규제 사실(EMR 인증제=기능성·상호운용성·보안성·ISMS-P 시 보안성 면제, DTx 솜즈/슬립큐, 비대면진료 법제화, 마이헬스웨이=FHIR)은 공개 출처로 직접 교차확인, 시장 수치는 미확인 표기. **적대적 검증 패스**(독립 에이전트·WebSearch 재확인) 후 교정 반영 — 비대면진료 시점(2025.12 통과)·DTx 2호 명칭(슬립큐/WELT-I)·임시등재, 권한상승(H1) 트리거 메커니즘·동적 attending 평가·Realtime 제외, 6개월 적색선 해소를 합성테스트 수준으로 한정, triage 배지 비-CDSS 선언, 진단코드 등급 하향.
...
3. 진단 특정자/중증도 다중 코드 확장 → 완료(48종). 후속: 진단 즐겨찾기·최근 사용.
...
- 제품군 분석·로드맵(포트폴리오, 코드 무관) → `docs/project-analysis.md`, `docs/product-roadmap.md`
