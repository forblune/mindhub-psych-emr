...
진단 DSM-5→ICD-10(KCD) 표준화 → 예약→진료→청구 워크플로 연동까지 완료.
**화면 7종(대시보드·예약·병동·통계·검색·청구·약품)**. 데이터는 전부 가상.
...
> 마이그레이션 `0001~0020`, E2E **40/40**, **화면 7종**(대시보드·예약·병동·통계·검색·청구·약품), 임상 데이터 4종 + 입원 CRUD + 신규 접수 + 예약·청구 + 약품·재고(입고·불출·등록·삭제, 처방 시 재고 자동 차감) + 담당의별 RLS + Realtime(큐·노트·처방·입원).
