...
> 마이그레이션 `0001~0007`, E2E **19/19**, 임상 데이터 4종(노트·처방·척도·검사) 전부 CRUD + 담당의별 RLS.
...
1. **Realtime** — `supabase.channel().on('postgres_changes', ...)` 로 대기열·노트 자동 갱신.
2. **권한 시연** — nurse/admin 계정으로 전체 조회 vs 담당의 격리 비교.
3. **KPI 집계 뷰** — `kpis` 테이블 → SQL view 로 대체(대기열에서 실시간 계산).
...
# SQL Editor: 0001 → 0002 → 0003 → 0004 → 0005 → 0006 → 0007 → seed 순서로 실행
# 앱에서 가입 → 로그인
