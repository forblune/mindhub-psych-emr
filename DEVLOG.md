...
KPI 집계뷰 → **화면 4종(대시보드·병동·통계·환자검색)** 까지 완료.
...
> 마이그레이션 `0001~0013`, E2E **27/27**, 화면 4종(대시보드·병동·통계·검색), 임상 데이터 4종 전부 CRUD + 입원 CRUD + 신규 접수 + 담당의별 RLS + Realtime(큐·노트·처방·입원).
...
- **E2E (Playwright/Chromium, mock 모드)**: `npm test` → **27/27 통과, 콘솔 에러 0**
  - 렌더 / KPI 집계(7) / 통계 지표 / 환자 검색·이동 / 새로고침 / 신규 진료 시작 / 입원·병동 네비·보드·필터 / 입원 등록·퇴원 / 환자 전환 / 탭4 / 테마 / 대기열 검색·정렬 / 노트·처방·척도·검사 CRUD
...
1. 예약 상태 모델링(예약 관리 화면).
2. 화면 추가: 청구·수납.
3. 환자 검색을 patients 테이블 직접 조회로(현재 오늘 큐+입원 범위).
...
# SQL Editor: 0001 → … → 0013 → seed 순서로 실행
# 앱에서 가입 → 로그인
...
- DB 스키마/정책 → `supabase/migrations/*.sql` (0001~0013)
- 인증 흐름 → `src/context/AuthContext.jsx`, `src/components/Login.jsx`
- 화면 전환 → `App.jsx` `view` (dashboard/ward/stats/search), 사이드바 `data/config.js`
- 화면 컴포넌트 → `src/components/**` (탭: `tabs/**`; 화면: `Ward`/`Stats`/`PatientSearch`/`NewVisit`)
