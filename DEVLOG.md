...
역할 기반 Auth/RLS → 검색·정렬 → **노트·처방·척도·검사 전부 CRUD** 까지 완료.
...
| `4795034` | 개발일지 정리 |
| (이번) | Realtime 대기열 자동 갱신 |

> 마이그레이션 `0001~0008`, E2E **19/19**, 임상 데이터 4종(노트·처방·척도·검사) 전부 CRUD + 담당의별 RLS + 대기열 Realtime.
...
## 구현 완료 (이번)
- **Realtime 대기열 자동 갱신** — `supabase.channel().on('postgres_changes', {table:'queue_entries'})` 구독 → 변경 시 큐 재조회(300ms 디바운스). RLS(`clinical_read=owns_patient`)로 본인 환자 변경만 수신. 구독되면 대기열 헤더에 **"● 실시간"** 펄스 배지. mock 모드/비로그인은 무동작·배지 없음.
  - `0008_realtime.sql`: `queue_entries` 를 `supabase_realtime` publication 에 추가(가드로 vanilla PG no-op, 멱등). **로컬 검증 완료**(없으면 no-op / 있으면 추가 / 재실행 중복无).
  - ⚠️ 실제 실시간 수신은 호스팅 Supabase 필요(로컬 검증 불가). publication SQL·구독 코드·RLS 필터는 검증됨.

## 아직 안 된 것
- 새로고침·"신규 진료 시작" 버튼 = **UI만, 동작 미구현**
- KPI는 큐레이션 값(집계 뷰 아님)
- 미검증(서비스 레이어, SQL 아님): 호스팅 GoTrue 이메일 인증, PostgREST 임베딩 응답 shape → 본인 Supabase에 올린 뒤 로그인 1회로 확인 권장

---

## 다음 단계 (우선순위)

1. **권한 시연** — nurse/admin 계정으로 전체 조회 vs 담당의 격리 비교.
2. **KPI 집계 뷰** — `kpis` 테이블 → SQL view 로 대체(대기열에서 실시간 계산).
3. Realtime 확장 — 노트/처방까지 구독(현재 대기열만).
4. 새로고침·"신규 진료 시작" 버튼 실동작.
5. 화면 추가: 입원 병동, 통계 지표, 청구·수납.
...
# SQL Editor: 0001 → … → 0008 → seed 순서로 실행
# 앱에서 가입 → 로그인
...
- DB 스키마/정책 → `supabase/migrations/*.sql` (0001~0007)
- 인증 흐름 → `src/context/AuthContext.jsx`, `src/components/Login.jsx`
- 화면 컴포넌트 → `src/components/**` (탭: `src/components/tabs/**`)
