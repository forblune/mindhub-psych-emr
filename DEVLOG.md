...
- **포트폴리오 문서 2종 추가** (`docs/`, 코드 무수정) — 다중 에이전트 워크플로로 자매 프로젝트 `forblune/mindhub-mvp`(환자측 마음기록)와 본 EMR을 교차 분석.
  ...
  - `docs/product-roadmap.md`: 6개월/1년 로드맵 · MVP→Beta→Production · 정신과 실도입 기술요소([보유·부분·미구현]) · 수익 모델. 규제 사실(EMR 인증제=기능성·상호운용성·보안성·ISMS-P 시 보안성 면제, DTx 솜즈/필로우Rx, 비대면진료 제한, 마이헬스웨이=FHIR)은 공개 출처로 직접 교차확인, 시장 수치는 미확인 표기. ⚠ 로드맵 워크플로의 자동 검증 단계는 세션 한도로 중단되어 작성자 셀프검증으로 보수 처리.
