...
export default function LabsTab({ detail, onAddLab, onDeleteLab }) {
        ...
        <form className="note-form" onSubmit={submit}>
          <div className="note-form-grid">
            <label className="note-field">
              <span>검사 분류</span>
              <input value={vals.group} onChange={(e) => set('group', e.target.value)} placeholder="대사 · 안전성 모니터링" />
            </label>
            <label className="note-field">
              <span>항목명 *</span>
      ...
      <table>
        ...
        <tbody>
          {detail.labs.map((group, gi) => (
            <Group key={group.group + gi} group={group} gi={gi} onRemove={remove} />
          ))}
        </tbody>
      </table>
    </div>
...
function Group({ group, gi, onRemove }) {
  return (
    <>
      <tr>
